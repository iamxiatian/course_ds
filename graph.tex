...
\begin{frame}[fragile]
  ...
  \begin{figure}[ht!]
    \includegraphics[width=0.35\textwidth]{figs/graph/Spanning_Tree_Idea.pdf}
  \end{figure}
\end{frame}
...
\begin{frame}[fragile]
  ...
  \begin{figure}[ht!]
    \includegraphics[width=0.9\textwidth]{figs/graph/PRIM.pdf}
  \end{figure}
\end{frame}
...
\begin{frame}[fragile, allowframebreaks]
  \frametitle{Prim算法实现}
  可使用两个数组动态地维护每个点到生成树T的点最短距离信息，提高找最短边的效率。
  \begin{itemize}
  \item lowcost：每个顶点到生成树中的最小代价
  \item closest：最小代价对应边的相邻节点
  \end{itemize}
  
  \newpage

  \begin{figure}[ht!]
    \includegraphics[width=0.9\textwidth]{figs/graph/PRIM2.pdf}
  \end{figure}
\end{frame}
