...
% 1. 根据26个英文字母的频度表, 构建哈夫曼算法，为每一个字母赋予一个唯一的最优编码。
% 2. 在notebook中能够输出每一个英文字母的编码结果，输出时每一个英文字母一行，格式为：字母:编码
% 3. 将字符串“hello world again” 写入程序当前目录下的文件"hello.txt"
% 4. 利用第1步构建的树，读取hello.txt文件中的内容并进行哈夫曼编码和压缩，压缩后的结果保存到"hello_huffman.dat"文件中
% 5. 读取文件"hello_huffman.dat"，并将其内容根据哈夫曼编码关系，还原出原始内容
% 5. 代码采用notebook编写，需要安装的依赖、程序说明、代码和注释均记录在该文件中，保存的文件名称为:  huffman_姓名.ipynb
% 6. 截止时间：5月7日23:00
% 7. 提交形式：以: 哈夫曼树-学号-姓名 为标题，ipynb文件为附件，发送邮件到助教邮箱。

% 得分点：
%  1. 邮件标题、文件命名等符合上面的要求 10%
%  2. 通过notebook文件，程序正常运行 20%
%  3. 对所有类和方法的注释 10%
%  4. 英文字符的哈夫曼编码正常输出 30%
%  5. “hello.txt“压缩和解压正常  30%
